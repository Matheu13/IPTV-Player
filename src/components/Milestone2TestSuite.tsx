...
export const Milestone2TestSuite: React.FC = () => {
  ...
  const runTests = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/m2/test-suite');
  ...
  return (
    <div id="m2-test-suite-container" className="space-y-6">
      {/* Header & Runner Bar */}
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-slate-100">
              Milestone 2 Playback Engine Automated Test Suite
            </h2>
          </div>
          <p className="text-xs text-slate-400 mt-1">
            Running 12 assertions across MPV IPC protocol framing, HWDEC fallback, deinterlacing, audio/subtitle selectors, connection limits, and stall auto-recovery.
          </p>
        </div>
        ...
        >
          ...
          {loading ? 'Executing Suite...' : 'Re-Run Milestone 2 Tests'}
        </button>
      </div>
